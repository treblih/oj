...
int bst_find(int wanted, int insert)
{
	int val;
	/* root has no parent */
	if (total == 0) goto insert;
	struct bst *child = bst + 1, *parent;
	while (child) {
		parent = child;
		val = child->val;
		if (val == wanted) return child - bst;
		else if (wanted < val) child = child->left;
		else child = child->right;
	}
	if (wanted < parent->val) parent->left = &bst[total+1]; 
	else parent->right = &bst[total+1];
insert:
	if (!insert) return 0;
